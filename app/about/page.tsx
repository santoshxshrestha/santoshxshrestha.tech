...
export default function About() {
  ...
  const infoData = [
    ...
    { label: 'Languages:', value: 'JavaScript, Rust, C, C++, Bash, Lua, Python' },
    ...
    { label: 'Interests:', value: 'Open Source, System Design, little bit of web' },
      ...
      <div className="terminal-content">
        ...
        <div className={`output-hidden ${showOutput ? '' : 'hidden'}`}>
          {infoData.map((info, index) => (
            <div key={index} className={`info-line ${visibleLines.includes(index) ? '' : 'hidden'}`}>
              <span className="label">{info.label}</span>
              {' '}
              <span className={`value ${info.highlight ? 'highlight' : ''}`}>{info.value}</span>
            </div>
          ))}
        </div>
